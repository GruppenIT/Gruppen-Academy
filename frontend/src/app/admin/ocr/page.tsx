...
import type { OCRUpload, OCRExtractedResponse, ParticipationEvaluationSummary, Question } from '@/types'
import {
...
export default function AdminOCRPage() {
  ...
  const handleUpload = async () => {
    if (!selectedParticipation || !uploadFile) return
    setUploading(true)
    setError('')
    try {
      await api.uploadOCRPdf(selectedParticipation, uploadFile)
  ...
  const openDetail = async (upload: OCRUpload) => {
    ...
    setError('')
    try {
      // Find the participation to get the journey questions
      ...
      {view === 'list' && (
        <>
          <div className="flex items-center justify-between mb-6">
            <div>
              <h2 className="text-lg font-semibold text-gray-900">Upload OCR</h2>
              <p className="text-sm text-gray-500">
                Envie PDFs digitalizados de jornadas presenciais para extrair respostas via OCR.
              </p>
            </div>
            <button
          ...
          {uploads.length === 0 ? (
            <div className="text-center py-12 text-gray-400">
              ...
              <p>Nenhum upload OCR encontrado.</p>
                    ...
                    return (
                      <tr key={upload.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-3 text-gray-600">
                          {part ? `${part.user_name} - ${part.journey_title}` : upload.participation_id.slice(0, 8)}
                        </td>
                        <td className="px-4 py-3">
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold">Enviar PDF Digitalizado</h3>
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Participacao</label>
                <select
                ...
                >
                  ...
                  {participations
                    .filter(p => !p.completed_at || p.total_responses === 0)
                    .map(p => (
                      <option key={p.participation_id} value={p.participation_id}>
                        {p.user_name} - {p.journey_title}
                      </option>
                    ))}
                </select>
              </div>
            ...
            <div className="flex justify-end gap-3 mt-6">
              ...
              <button
                ...
                disabled={!selectedParticipation || !uploadFile || uploading}
